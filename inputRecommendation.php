...
class Recommendation_long_term_by_input {
  ...
  /* find artists indirectly releted with the user */
    public function find_artist () {
      var_dump("pASSOU POR AQUI!");
      $connect_vai = mysql_connect("localhost","root","");
      ...
      $query_db = "SELECT DISTINCT id_fb FROM artista WHERE artista.id_fb IS NOT NULL AND LOWER(nome_artista) LIKE LOWER('%".mysql_real_escape_string(strtolower($this->input))."%')" ;
      $artistas_input_res = mysql_query($query_db, $connect_vai);
  ...
  private function long_term($num) {
    ...
    if(is_array($this->artistas)) {
          ...
          $events_res = mysql_query($query_db, $connect_vai);
